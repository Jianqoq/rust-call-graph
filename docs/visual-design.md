...
Depth is read from the source being inspected, not merely from the first route that revealed a shared function. A shared callee reached again from a second-column Source belongs with the third-column targets; only that target column permutes on hover. Recursive cycles stay spatially bounded rather than manufacturing endlessly deeper columns.
