...
- `pnpm test` — 55 unit tests cover Struct/Enum scanning, trait and generic impl association, compact shared-column layout, movable column-x preservation, cycles, bounded recent-target cell permutation, active-chain retention and multi-hop oldest-to-newest layout application, unrelated-source reset and recent-history deduplication, whole-column drag preview and vertical row swapping, expanded-source horizontal/vertical collision reflow and restoration, target-source focus preservation, origin-source focus dismissal, long-name source-handle clamping, delayed language-hover requests, native-hierarchy hover rendering without nested code rectangles, safe hover-card rendering, direction branch visibility with shared-node retention, exact-pin suppression of aggregate node hover, exclusive relationship pinning, qualified-path range narrowing and icon-free source rendering, semantic and lexical source token segmentation, comment/string exclusion, lifetime-versus-character disambiguation, raw-identifier handling, dedicated source-theme token mapping, compact Function Node rendering without a signature summary, aggregate-versus-exact source endpoint selection, same-kind fan-out bundling and style-lane separation, multiple source call sites, and header-only node dragging.
...
- `pnpm package` — produces a 183.42 KB installable VSIX containing only the extension bundle, Webview bundle, styles, README, changelog, and license.
...
- In a two-hop reproduction, hovering the middle function moved it from `translate(710px, 183px)` to `translate(710px, 0px)`. Expanding its Source and hovering `current_exe` kept that middle node at `translate(710px, 0px)` while arranging the next target column, instead of returning it to the baseline row.
...
- Source language Hover renders zero provider labels, zero language labels, one continuous transparent code region with a 0 px border and padding, and wrapping long signatures inside the single outer editor-Hover surface.
