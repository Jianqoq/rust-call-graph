...
- `pnpm package` — produces a 181.15 KB installable VSIX containing only the extension bundle, Webview bundle, styles, README, changelog, and license.
...
- Dragging `submit_order` diagonally left and below the grid keeps x = 710, clamps y to the outer row, and snaps on release to `translate(710px, 183px)` while the displaced node takes the vacated legal cell.
