...
- The relationship origin is an anchored cell during recent-target ordering. When a target occupies the same rank column as its expanded origin, the origin keeps its exact x/y position and is excluded from both candidate target cells and remaining-node redistribution.
...
27. Hovering a source relationship never changes the expanded origin node's grid position, including when origin and target share a column; target recency still permutes the other legal cells.
