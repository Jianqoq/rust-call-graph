...
- When a Function Node is reachable through more than one outgoing depth, it occupies the deepest acyclic rank. Strongly connected recursive functions retain a bounded internal ordering. Historical column positions may move a complete rank but may not collapse two distinct ranks onto the same x coordinate.
...
27. Hovering a relationship from a second-column Source reorders only third-column targets; a target also called directly by the Entry Function does not remain in or reorder the second column, and recursive cycles remain bounded.
