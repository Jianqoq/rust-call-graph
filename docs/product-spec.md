...
- Pausing over a semantic source token requests hover information from the active VS Code language provider at the token's real workspace URI and position. Rust signatures and documentation appear in a delayed, theme-aware, non-executable hover card. Provider content follows VS Code's compact hierarchy: adjacent Rust code fences form one wrapping code region without nested rectangles or repeated provider/language labels.
...
27. After one Source promotes a middle Function Node beside it, expanding that middle node and hovering a second-hop relationship keeps the middle node at the promoted x/y position while reordering the second hop's target column.
