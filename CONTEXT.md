...
**Recent Target**:
A Function Node most recently reached by hovering or selecting a Call Site or Function Reference. Within its shared rank column, newer Recent Targets occupy cells closer to the source and older ones progressively farther away. Each source retains its ordering, so a promoted target can become the next source without losing its prior placement; manually translating a column carries every Recent Target with it.
