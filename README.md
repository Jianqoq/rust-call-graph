...
- Browsing can continue from a promoted target without losing the earlier placement. If `inspect` first brings an installed-function node beside it, expanding that middle node and hovering one of its callees reorders the next target column while the middle node remains beside `inspect`.
